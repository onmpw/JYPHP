...
class Common
{
     ...
     * @param string $key
     */
    public static function UrlFilter(&$val, $key)
    {
     ...
     * @param array $arr
     */
    public static function parseEmpty(array &$arr)
    {
        foreach ($arr as $k => $v) {
            if (empty($v)) {
                unset($arr[$k]);
     ...
     */
    public static function smartyConstantFilter($tplName)
    {
        return preg_replace('/__([a-zA-Z]*)__/', '{$smarty.const.\\1}', $tplName);
    }
    ...
    public static function session($name = '', $value = '')
    {
        ...
        if (is_array($name)) { // session初始化 在session_start 之前调用
            if (isset($name['prefix'])) {
                self::C(['SESSION_PREFIX', $name['prefix']]);
            }
