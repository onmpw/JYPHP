<?php
/**
 * 此验证码类在使用的过程中存在使用上的bug 比如 产生验证码的时候需要实例化一个验证码类 而在验证的时候还需要产生一个验证码类
* 此时就会面临两个验证码的实例对象的配置有可能不一样 比如说 过期时间  如果在产生的时候设置了过期时间  而在验证的时候没有设置
* 那么就会使用默认的过期时间 那么两个对象的时间就会不一样 那和你想要的结果就会产生差别
*/
namespace Lib;
...
class CheckCode {
    ...
    private $_error_code = NULL;  //验证码错误代码 表示验证码的错误类型  这是自己加的变量
    ...
    /**
     * 这是自己添加的函数 用来设置验证码错误类型码
     * 验证码错误类型码    0 表示 输入的验证码为空   1表示 当前验证码已失效  2表示 当前验证码已过期
     * 说明： 验证码已失效和验证码已过期 不等同
     *      验证码已失效 分两种情况
     *          1、可能是验证码已经正确验证过一次 此时会将session中保存当前验证码置为空
     *          2、可能是验证码生成以后，很长时间没有输入验证，当输入 验证的时候 验证码已经过期失效
     *      验证码过期 只是验证码失效的第二种情况
     *
     * @access private
     * @param string $code
...
//                 imagettftext($this->_image, $this->fontSize, mt_rand(-40, 40), $codeNX, $this->fontSize*1.6, $this->_color, $this->fontttf, $code[$i]);
        ...
        //自己修改的代码
        $secode['verify_expire']=$this->config['expire']; //这是将过期时间保存起来
